...
# Obtain the values out of X_R: X_R_b[0] < X_R_b[1] < X_R_h[0] < X_R_h[1]
def outregion(X_R_h, X_R_b, siz):
    l=[]

    i=X_R_h[0]
    while(i>=X_R_h[0] and i<=X_R_h[1]):
        j=X_R_b[0]
        while(j<=X_R_b[1] and j>=X_R_b[0]):
            l.append([i,i,j,j])
            j = j + siz
        j=X_R_h[0]
        while(j<=X_R_h[1] and j>=X_R_h[0]):
            l.append([i,i,j,j])
            j = j + siz
        i=i+siz

    i=X_R_b[0]
    while(i>=X_R_b[0] and i<=X_R_b[1]):
        j=X_R_b[0]
        while(j<=X_R_b[1] and j>=X_R_b[0]):
            l.append([i,i,j,j])
            j = j + siz
...
def com_coff(X_dis, U_dis,DFA,K):
    ...
    # Condition 2
    # states outseid the legal region
    out_distance=0.5
    siz=0.1
    outer=outregion([X_max+siz, X_max+out_distance],
                    [X_min-out_distance, X_min-siz],siz)
    ...
    for x_c in X_dis:
        ...
        for q_c in Qc:  # Go through the possible current states
            ...
            ind = DFA.index(q_c)
            ind_n = DFA.index(q_n)
            con.append(coe[ind] @ mono >= coe[ind_n] @ mono_n)
